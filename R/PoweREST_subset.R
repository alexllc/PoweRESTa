...
{ if (!requireNamespace("Seurat", quietly = TRUE)) {
  ...
  txt<-paste0('if (length(unique(Seurat_obj@meta.data$',cond,'))!=2) {stop("The number of condition groups should be 2.")}')
  eval(parse(text=txt))
  ...
  for (i in 1:n1) {
    ...
    Subset<-subset_Seurat(x = Seurat_obj, subset = PoweREST_Group == 1)
    if ('test.use' %in% dnm){
      x<-FindMarkers(Subset, ident.1 = name1, ident.2= name2,verbose=FALSE,logfc.threshold = logfc.threshold,min.pct = min.pct,...) }
    else{ x<-FindMarkers(Subset, ident.1 = name1, ident.2= name2,verbose=FALSE,logfc.threshold = logfc.threshold,min.pct = min.pct) }
